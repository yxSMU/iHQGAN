...
import os
import numpy as np
from models.generator import ClassicalGAN1
from models.generator2 import ClassicalGAN2


def main():
    iter_num = 50

    out_dir = r'F:\conv\canny0\A2B'
    out_dir_2 = r'F:\conv\canny0\B2A'
    out_dir_3 = r'F:\conv\canny0\A_real'
    out_dir_4 = r'F:\conv\canny0\B_real'
    ...
    dataset_st = r"E:\Acdamic\Data\DataMake\Mnist_C\Canny\Canny_0_shuffle"
    data = ImageDataset(dataset_st)
    dloader = DataLoader(data, batch_size=10,
                         shuffle=False, num_workers=1)  # 数据集
    ...
    total_params = sum(p.numel()
                       for p in generator_2.parameters() if p.requires_grad)
    print("Total number of parameters in the quantum generator: ", total_params)

    optimizer_G = torch.optim.Adam(itertools.chain(generator_1.parameters(), generator_2.parameters()), lr=lr_G,
                                   betas=(b1, b2))
    ...
    for epoch in range(iter_num):
        for i, batch in enumerate(dloader):
            ...
            image_aba = generator_2(pred_images_B)
            # loss_aba = criterion_cycle(real_A, image_aba)
            ...
            # loss_bab = criterion_cycle(real_B, image_bab)*10
            ...
            g_loss_sum = gloss_b2a + gloss_a2b
            g_loss_sum.backward()
            optimizer_G.step()
            ...
            if batches_done in batch_indices_to_save:
                ...
                torch.save(generator_2.state_dict(), os.path.join(
                    out_dir, f'generator_2-batch{batches_done}.pt'))
